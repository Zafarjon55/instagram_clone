...
from .serializers import UserSerializers,ChangeUserPhotoSerializers,LoginSerializers
from .models import User, NEW, CODE_VERIFIED,VIA_EMAIL,VIA_PHONE
from shared.utils import send_email
from shared.utils import send_phone
...
class GetNewVerifyCode(APIView):
    ...
    def check_verify(user):
        verifies = user.verifycode.filter(expiration_get=datetime.now(),is_confirmed=False)
        
        if verifies.exsist():
            data = {
                'message':"kechirasiz sizning kodingiz hali faol"
            }
            raise ValidationError(data)
...
class UserChangeInformation(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        data = {
            "message":"User muvofaqqiyatli yangilandi! ",
...
class ChangeUserPhoto(APIView):
    ...
    def put(self,request,*args,**kwargs):
        serializer = ChangeUserPhotoSerializers(data=request)
        if serializer.is_valid():
            user = request.user
            serializer.update(user.serializer.validated_data)
            return Response(
                {
                    'message':'Rasm ornatildi ! '
                }
            )
